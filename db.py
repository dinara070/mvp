...
-- ---------------------------------------------------------------------
-- Аудит дій (критично для держпідприємств та енергетики)
-- ---------------------------------------------------------------------

CREATE TABLE IF NOT EXISTS audit_log (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    user_id INTEGER REFERENCES users(id),
    user_name TEXT,                   -- денормалізовано, щоб лог лишався читабельним навіть якщо юзера видалять
    action_type TEXT NOT NULL,        -- LOGIN | VIEW_DOCUMENT | TAKE_TEST | CHANGE_STATUS | CREATE | UPDATE | DELETE | CHAT_QUERY | GENERATE_QUIZ
    object_type TEXT,                 -- kb_document | leave_request | certificate_request | equipment_request | substation | user | safety_instruction ...
    object_id INTEGER,
    description TEXT,
    created_at TEXT NOT NULL
);
...
-- ---------------------------------------------------------------------
-- Чат-історія RAG-асистента (для відтворюваності відповідей та аудиту)
-- ---------------------------------------------------------------------
...
CREATE TABLE IF NOT EXISTS chat_history (
    ...
    sources TEXT,                     -- JSON-список назв документів, використаних для відповіді
    llm_used INTEGER DEFAULT 0,       -- 1 = відповідь згенерована LLM, 0 = екстрактивний fallback без LLM
    created_at TEXT NOT NULL
);
"""
