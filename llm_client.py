...
не встановлена, чи виникла мережева помилка) — усі функції повертають None
або кидають LLMUnavailableError, і виклики в rag_chat.py / quiz_generator.py
...
def chat_completion(system_prompt, user_prompt, max_tokens=700, temperature=0.2, json_mode=False):
    ...
    try:
        ...
        raise LLMUnavailableError(f"Помилка звернення до LLM: {e}") from e


def chat_completion_json(system_prompt, user_prompt, max_tokens=1200, temperature=0.2):
    ...
    raw = chat_completion(system_prompt, user_prompt, max_tokens=max_tokens,
